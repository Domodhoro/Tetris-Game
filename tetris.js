class Tetris {
    ...
    moveTetromino = function(right) {
        ...
        const x = this.fallingTetromino.x, y = this.fallingTetromino.y; 
        ...
        if (this.fallingTetromino === null) {
            ...
            shape.map((row, i) => {
                row.map((cell, j) => {
                    ...
                    if (u >= 0 && u < columns && v < rows && cell > 0) {
                        this.grid[v][u] = shape[i][j];
                    }
                })
            })
    ...
    rotateTetromino = function() {
        if (this.fallingTetromino !== null) {
            ...
            if (!this.collision(this.fallingTetromino.x, this.fallingTetromino.y, shape)) {
                this.fallingTetromino.shape = shape;
            }
        }
